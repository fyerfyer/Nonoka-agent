...
_TOOL_TASKS: tuple[dict[str, Any], ...] = (
  ...
  {"id": "delete-temp", "prompt": "Remove obsolete.tmp, add archive.txt containing exactly `archived` with no trailing newline, and run a directory listing to verify both changes.", "files": {"obsolete.tmp": "discard\n"}, "expected": {"archive.txt": "archived"}, "absent": ["obsolete.tmp"], "required_tools": ["delete_file", "write_file", "list_dir"]},
